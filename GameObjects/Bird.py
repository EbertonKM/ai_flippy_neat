...
class Bird:
    ...
    def load_frame(self): # Completamente errado
        for i in (1, 3):
            self.frames = pygame.image.load(os.path.join(config.TEXTURE_DIR, f"bird_{self.color}{i}.png")).convert_alpha()
        self.cframes = self.frames
        return self.cframes
    
    def move(self): # Falta aplicar isso de fato ao sprite
